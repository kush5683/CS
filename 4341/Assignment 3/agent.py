...
class Agent:
    ...
    def calc_heuristic(self, number, coord,curr_direction):
        ...
        if number == 1:
            ...
            h = diff_r + diff_c
            turn_cost = 0
            if goal[0]!=coord[0] and (curr_direction=='north' or curr_direction=='south'):
                turn_cost += 1
            elif goal[1]!=coord[0] and (curr_direction=='east' or curr_direction=='west'):
                turn_cost += 1

            if curr_direction == 'north' and goal[0]>coord[0]:
                turn_cost += 1
            elif curr_direction == 'south' and goal[0]<coord[0]:
                turn_cost += 1
            elif curr_direction == 'west' and goal[1]>coord[1]:
                turn_cost += 1
            elif curr_direction == 'east' and goal[1]<coord[1]:
                turn_cost += 1
            h += turn_cost
            if number == 6:
                h *= 3
            if number == 7:
                sld = math.sqrt(abs(coord[0] - goal[0]) ** 2 + abs(coord[1] - goal[1]) ** 2)
                h = 2.7011*h + 0.7299*sld+0.642 
        ...
        while len(OPEN) > 0:
            ...
            if self.board.get_cost(curr.row, curr.col) == -1:  # check if goal reached. if reached print path
                ...
                for step in path[::-1]:
                    ...
                    score = 100 - (curr.f + 1)
                    # print(step)
                return score, len_of_path, nodes_expanded, actions, path
            OPEN.remove(curr)
            VISITED.append(curr)
            for option in options_per_node:
                ...
                if option == "FORWARD":
                    ...
                    if cost_of_move == 0:
                        continue
                    new_heuristic = self.calc_heuristic(heuristic, new_pos,curr.direction)
                    curr.add_edge(
                    ...
                    if cost_of_move == 0:
                        continue
                    new_heuristic = self.calc_heuristic(heuristic, new_pos,curr.direction)
                    new_node = node.Node(new_pos[0], new_pos[1], curr, new_heuristic, "BASH", g=curr.g + cost_of_move,
                                  direction=curr.direction)
                    curr.add_edge(new_node)

                    # forward = self._move(curr_pos=[new_node.row, new_node.col], curr_dir=new_node.direction)
                    # new_pos = forward[0]
                    # cost_of_move = forward[1]
                    # if cost_of_move == 0:
                    #     continue
                    # new_heuristic = self.calc_heuristic(heuristic, new_pos, new_node.direction)
                    # new_node.add_edge(
                    #     node.Node(new_pos[0], new_pos[1], new_node, new_heuristic, "FORWARD",
                    #               g=new_node.g + cost_of_move,
