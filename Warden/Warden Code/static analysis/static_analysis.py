...
class FileLoader:
    ...
    def get_calls(self, file: str) -> list[function]:
        ...
        function_call_regex = "(\w+)\s*[(]"
        calls = self.remove_non_functions(
            set(regex.findall(function_call_regex, whole)))
        return calls

    def remove_non_functions(self, functions: set[function]) -> set[function]:
...
if __name__ == "__main__":
    file_loader = FileLoader(
        "/home/student/sus-stuff/mqp_sus_history/sus-manager/wp-versions/wp-511-ubuntu/nginx-fpm-build",
        ".php",
    )
